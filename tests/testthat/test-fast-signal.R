...
test_that("match_phylo_data reports matched and removed species", {
  set.seed(111)
  tree <- ape::rtree(12)
  ...
  m <- match_phylo_data(tree, X, verbose = FALSE)
...
test_that("lambda batch matches phytools", {
  ...
  expect_lt(max(abs(res$logL_fast - ref_logL), na.rm = TRUE), 1e-5)
...
test_that("fast_ace matches ape::ace for discrete ER and ARD likelihoods", {
  ...
  expect_lt(max(abs(ref_er$rates - fast_er$rates)), 1e-8)
...
test_that("fast_signal, fast_d, and fast_delta are separate entry points", {
  set.seed(23)
  tree <- ape::rtree(30)
  y <- stats::rnorm(30)
  ...
  expect_error(
    fast_signal(tree, z, method = "D", nsim = 20, verbose = FALSE),
    "should be one of"
  )
})
